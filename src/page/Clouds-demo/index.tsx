...
function Puffycloud({ seed, vec = new THREE.Vector3(), ...props }: any) {
  const api = useRef() as any;
  const light = useRef() as any;
  const rig = useContext(context) as any;
  const [flash] = useState(
    () => new random.FlashGen({ count: 10, minDuration: 40, maxDuration: 200 }),
  );
  const contact = (payload: any) =>
